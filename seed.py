...
def seed_table(table_name):
    ...
    try:
        duck.execute(
            f"CREATE TABLE temp_{table_name} AS SELECT * FROM read_parquet('{s3_path}', union_by_name=true);"
        )
    except:
    ...
    duck.execute(
        f"CREATE TABLE pg.marketing.{table_name} AS SELECT * FROM temp_{table_name};"
    )

    # Verify
    pg_count = duck.execute(
        f"SELECT COUNT(*) FROM pg.marketing.{table_name}"
    ).fetchone()[0]
...
def main():
    ...
    print(
        f"Target: {POSTGRES_CONFIG['host']}:{POSTGRES_CONFIG['port']}/{POSTGRES_CONFIG['database']}"
    )
